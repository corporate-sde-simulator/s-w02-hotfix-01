...
> **Welcome to Service-Track Week 2, Hotfix 1!**
...
1. **Read the comments** at the top of `userRepository.py` - they describe the problem
...
| File | Purpose |
|------|---------|
| `userRepository.py` | The code with bugs - **fix this file** |
...
### Bug #1
**What's wrong:** SQL Injection vulnerability â€” string concatenation in SQL

**How to find it:** Search for `BUG` in `userRepository.py` - the comments around each bug explain what's broken.

### Bug #2
**What's wrong:** Same pattern â€” string formatting instead of parameterized query

**How to find it:** Search for `BUG` in `userRepository.py` - the comments around each bug explain what's broken.

### Bug #3
**What's wrong:** Again, string formatting
...
1. **Read the top comment block** in `userRepository.py` carefully - it has:
